...
from sfivn.feature_extraction.scenes150.mit_semseg.models import (
    ModelBuilder,
    SegmentationModule,
)
...
def extract_feature(
    segmentation_module: SegmentationModule, image_path: str, cuda: bool = False, get_top_only:int=20
):
    ...
    get_list_tuple= zip_percentage_segment_sorted[:get_top_only]
    return {
        key: value for key,value in get_list_tuple
    }


def extract_feature_full(
    segmentation_module: SegmentationModule, image_path: str, cuda: bool = False
):
    segmentation_module.eval()
    if cuda:
        segmentation_module.to(torch.device("cuda:0"))
